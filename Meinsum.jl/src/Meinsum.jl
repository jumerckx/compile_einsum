...
macro meinsum(ex)
    _meinsum(ex) # true, false, false
end

function _meinsum(expr::Expr, inbounds = true, simd = false, threads = false)
...
function nest_loops(expr::Expr,
    ...
    for j = 1:length(index_names)
        if j == length(index_names) && accumulator !== nothing
            expr = nest_loop(expr, index_names[j], axis_expressions[j], accumulator)
        else
...
function nest_loop(expr::Expr, index_name::Symbol, axis_expression::Expr, accumulator::Tuple{Symbol, Symbol})
    @gensym loopbody_begin
    @gensym loopbody_end
    loop = quote
        $index_name, $(accumulator[1]) = Brutus.begin_for($(accumulator[2])(0), 1, $axis_expression)
        @goto $loopbody_begin
            @label $loopbody_begin
            $expr
            $(accumulator[1]) = Brutus.yield_for($(accumulator[1]))
            @goto $loopbody_end
        @label $loopbody_end
    end
    return loop
...
function nest_loop(expr::Expr, index_name::Symbol, axis_expression::Expr)
    ...
    @gensym loopbody_end
    loop = quote
        $index_name = Brutus.begin_for(1, $axis_expression)
        @goto $loopbody_begin
            @label $loopbody_begin
            $expr
            Brutus.yield_for()
